...
row_ha = rowAnnotation(primate_subclass = primate_subclass,
                              col = list(primate_subclass = celltype_color_palette), show_legend = F)
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/metaneighbor_plots/annot_primate_subclass_mouse_metacluster_heatmap.pdf',
    useDingbats = F, height = 8, width = 20)
ht_mc = ComplexHeatmap::Heatmap(test_metasubclass_hits, name = 'AUROC', col = auroc_cols,
                        row_order = order.dendrogram(row_dendrogram), cluster_rows = row_dendrogram,
                        column_order = order.dendrogram(col_dendrogram), cluster_columns = col_dendrogram,
...
mouse_match_prim_sub %>% filter(Var2 == 'zeng_10xv2_wba|Sst Chodl Gaba_4')
...
ht_mc = ComplexHeatmap::Heatmap(test_metasubclass_hits, name = 'AUROC', col = auroc_cols,
                        row_order = order.dendrogram(row_dendrogram), cluster_rows = row_dendrogram, show_row_dend = F,
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/metaneighbor_plots/custom_annot_primate_subclass_mouse_metacluster_heatmap.pdf',
    useDingbats = F, height = 8, width = 20)
ht_mc = ComplexHeatmap::Heatmap(test_metasubclass_hits, name = 'AUROC', col = auroc_cols,
                        row_order = order.dendrogram(row_dendrogram), cluster_rows = row_dendrogram, show_row_dend = F,
                        column_order = custom_order, cluster_columns = F,
...
FindIntegrationAnchors took almost 6.6 hours
...
DimPlot(interneuron.combined, group.by = 'study')
...
ggplot(temp_exp_df, aes(x = umap_1, y = umap_2, color = orthologous_subclass)) + geom_point(size = .25, alpha = .5) + 
  scale_color_manual(values = celltype_color_palette)
ggplot(temp_exp_df, aes(x = umap_1, y = umap_2, color = sncg_zscore)) + geom_point(size = .25, alpha = .5) + 
...
ggplot(temp_exp_df, aes(x = umap_1, y = umap_2, color = stac_zscore)) + geom_point(size = .25, alpha = .5) + 
  scale_color_gradient2(low = '#313695', mid ='grey', high = '#A50026', midpoint = 0 )
ggplot(temp_exp_df, aes(x = umap_1, y = umap_2, color = pax6_zscore)) + geom_point(size = .25, alpha = .5) + 
  scale_color_gradient2(low = '#313695', mid ='grey', high = '#A50026', midpoint = 0 )
...
all_metadata %>% filter(cell_cluster == 'Serfpin1') %>% group_by(study) %>% summarise(n = n())
...
primate_sub_comp_df = all_metadata  %>% group_by(primate_subclass, study) %>% summarise(n = n())
mouse_sub_comp_df = all_metadata  %>% group_by(cell_subclass, study) %>% summarise(n = n())


# Stacked + percent
ggplot(primate_sub_comp_df, aes(fill=study, y=n, x=primate_subclass)) + 
    geom_bar(position="fill", stat="identity") + ylab('% cell-type') + ggtitle('Study contribution to orthologous subclasses')

ggplot(mouse_sub_comp_df, aes(fill=study, y=n, x=cell_subclass)) + 
    geom_bar(position="fill", stat="identity") + ylab('% cell-type') + ggtitle('Study contribution to original subclasses')

primate_sub_comp_df %>% group_by(primate_subclass) %>% summarise(n = n()) 
