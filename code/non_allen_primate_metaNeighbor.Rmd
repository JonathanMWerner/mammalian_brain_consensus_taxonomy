...
Trying a metaneighbor when grouping the primate clusters at the subclass level
...
###############################################################
And now the caglayan dataset metaneighbor for mouse and primates
...
caglayan_gene_index = rownames(caglayan_gaba_data) %in% intersect_genes

human_cell_index = caglayan_gaba_data$Species == 'human'
chimp_cell_index = caglayan_gaba_data$Species == 'chimp'
macaque_cell_index = caglayan_gaba_data$Species == 'macaque'


caglayan_human = caglayan_gaba_data[caglayan_gene_index, human_cell_index]
caglayan_chimp = caglayan_gaba_data[caglayan_gene_index, chimp_cell_index]
caglayan_macaque = caglayan_gaba_data[caglayan_gene_index, macaque_cell_index]

rm(caglayan_gaba_data)
gc()
...
human_meta = data.frame(cell_cluster = as.character(caglayan_human$seurat_clusters), cell_subclass = NA, primate_subclass = NA) %>% DataFrame()
colData(caglayan_human) = human_meta

chimp_meta = data.frame(cell_cluster = as.character(caglayan_chimp$seurat_clusters), cell_subclass = NA, primate_subclass = NA) %>% DataFrame()
colData(caglayan_chimp) = chimp_meta

macaque_meta = data.frame(cell_cluster = as.character(caglayan_macaque$seurat_clusters), cell_subclass = NA, primate_subclass = NA) %>% DataFrame()
colData(caglayan_macaque) = macaque_meta
...
caglayan_mouse_sce_list = list(
  ...
  zeng_10xv3_wba = zeng_10xv3_wba,
  macosko_wba = macosko_wba, 
  caglayan_human = caglayan_human, 
  caglayan_chimp = caglayan_chimp,
  caglayan_macaque = caglayan_macaque
)
...
```{r}
merged_caglayan_mouse = mergeSCE(caglayan_mouse_sce_list)
dim(merged_caglayan_mouse)
colData(merged_caglayan_mouse)
...
rm(caglayan_mouse_sce_list)
gc()

merged_caglayan_mouse
```

```{r}

saveRDS(merged_caglayan_mouse, 
        file = '/inkwell03/werner/mouse_brain_consensus_taxonomy/non_allen_data/merged_caglayan_mouse_sce.rds')

```

```{r}

merged_caglayan_mouse = readRDS(file =  '/inkwell03/werner/mouse_brain_consensus_taxonomy/non_allen_data/merged_caglayan_mouse_sce.rds')
...
```{r}
start = Sys.time()
global_hvgs = variableGenes(dat = merged_caglayan_mouse , exp_labels = merged_caglayan_mouse$study_id, min_recurrence = 5)
length(global_hvgs)
Sys.time() - start
```
...
table(grepl('caglayan', merged_caglayan_mouse$study_id))


mouse_subclass_primate_cluster = merged_caglayan_mouse$primate_subclass
primate_index = grepl('caglayan', merged_caglayan_mouse$study_id)
mouse_subclass_primate_cluster[primate_index] = as.character(merged_caglayan_mouse$cell_cluster)[primate_index]

table(mouse_subclass_primate_cluster)


merged_caglayan_mouse$mouse_subclass_primate_cluster = mouse_subclass_primate_cluster
...
```{r}
start = Sys.time()
caglayan_mouse_subclass_best_hits = MetaNeighborUS(var_genes = global_hvgs, 
                              dat = merged_caglayan_mouse, 
                              study_id = merged_caglayan_mouse$study_id,
                              cell_type = merged_caglayan_mouse$mouse_subclass_primate_cluster,
                              fast_version = T,
                              one_vs_best = T, symmetric_output = F)
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_mouse_subclass_1vsbest.pdf', 
    useDingbats = F, height = 25, width = 25)
plotHeatmap(caglayan_mouse_subclass_best_hits, cex = .5)
dev.off()



start = Sys.time()
caglayan_mouse_subclass_aurocs = MetaNeighborUS(var_genes = global_hvgs, 
                              dat = merged_caglayan_mouse, 
                              study_id = merged_caglayan_mouse$study_id,
                              cell_type = merged_caglayan_mouse$mouse_subclass_primate_cluster,
                              fast_version = T)
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_mouse_subclass_aurocs.pdf', 
    useDingbats = F, height = 25, width = 25)
plotHeatmap(caglayan_mouse_subclass_aurocs, cex = .2)
dev.off()
...
saveRDS(caglayan_mouse_subclass_best_hits, 
        file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_mouse_subclass_1vsbestMN.rds')

saveRDS(caglayan_mouse_subclass_aurocs, 
        file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_mouse_subclass_full_aurocs.rds')

```

```{r}

caglayan_mouse_subclass_best_hits = readRDS(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_mouse_subclass_1vsbestMN.rds')


caglayan_mouse_subclass_aurocs = readRDS(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_mouse_subclass_full_aurocs.rds')
...
cluster_graph = makeClusterGraph(caglayan_mouse_subclass_best_hits , low_threshold = .6)

plotClusterGraph(cluster_graph, merged_caglayan_mouse$study_id, 
                 merged_caglayan_mouse$mouse_subclass_primate_cluster, size_factor = 3)
...
mouse_subclass_mappings_df = as.data.frame(colData(merged_caglayan_mouse)) %>% filter(!grepl('caglayan', study_id)) %>%
  mutate(study_clust_label = paste(study_id, mouse_subclass_primate_cluster, sep = '|')) %>%
  filter(!duplicated(study_clust_label))
...
vertex_study <- factor(igraph::V(temp_cluster_graph)$name)
#Grab the primate cluster names 
primate_clusts = as.character(vertex_study[grepl('caglayan', vertex_study)])
#And add those to the vertex colors
primate_colors = rep('grey', length = length(primate_clusts))
names(primate_colors) = primate_clusts

#Grab the mouse subclasses
#mouse_subclasses = as.character(vertex_study[!grepl('caglayan', vertex_study)])
#Just the subclass labels
#sub_label = sapply(strsplit(mouse_subclasses, split = '|', fixed = T), '[[', 2)
#mouse_colors = celltype_color_palette[sub_label]
#names(mouse_colors) = mouse_subclasses
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_mouse_subclass_1vsbest_cluster_graph.pdf', 
    useDingbats = F, height = 10, width = 10)
plot(temp_cluster_graph,
     vertex.size = 8,
     vertex.label.cex=1.5,
...
intersect_genes = intersect(allen_primate_genes, caglayan_genes)
length(intersect_genes)
...
human_meta = data.frame(cell_cluster = as.character(caglayan_human$seurat_clusters), cell_subclass = NA) %>% DataFrame()
colData(caglayan_human) = human_meta
chimp_meta = data.frame(cell_cluster = as.character(caglayan_chimp$seurat_clusters), cell_subclass = NA) %>% DataFrame()
colData(caglayan_chimp) = chimp_meta
macaque_meta = data.frame(cell_cluster = as.character(caglayan_macaque$seurat_clusters), cell_subclass = NA) %>% DataFrame()
colData(caglayan_macaque) = macaque_meta
...
caglayan_primate_sce_list = list(
  human_10x = human_10x,
  human_ss = human_ss,
  chimp_10x = chimp_10x, 
  chimp_ss = chimp_ss,
...
```{r}
merged_caglayan_primate = mergeSCE(caglayan_primate_sce_list)
dim(merged_caglayan_primate)
...
rm(caglayan_primate_sce_list)
gc()

merged_caglayan_primate
```

```{r}

saveRDS(merged_caglayan_primate, 
        file = '/inkwell03/werner/mouse_brain_consensus_taxonomy/non_allen_data/merged_caglayan_primate_sce.rds')

```

```{r}

merged_caglayan_primate = readRDS(file =  '/inkwell03/werner/mouse_brain_consensus_taxonomy/non_allen_data/merged_caglayan_primate_sce.rds')
...
```{r}
start = Sys.time()
global_hvgs = variableGenes(dat = merged_caglayan_primate , exp_labels = merged_caglayan_primate$study_id, min_recurrence = 5)
length(global_hvgs)
Sys.time() - start
```
...
table(grepl('caglayan', merged_caglayan_primate$study_id))


primate_subclass_primate_cluster = merged_caglayan_primate$cell_subclass
primate_index = grepl('caglayan', merged_caglayan_primate$study_id)
primate_subclass_primate_cluster[primate_index] = as.character(merged_caglayan_primate$cell_cluster)[primate_index]

table(primate_subclass_primate_cluster)


merged_caglayan_primate$primate_subclass_primate_cluster = primate_subclass_primate_cluster
...
```{r}
start = Sys.time()
caglayan_primate_subclass_best_hits = MetaNeighborUS(var_genes = global_hvgs, 
                              dat = merged_caglayan_primate, 
                              study_id = merged_caglayan_primate$study_id,
                              cell_type = merged_caglayan_primate$primate_subclass_primate_cluster,
                              fast_version = T,
                              one_vs_best = T, symmetric_output = F)
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_primate_subclass_1vsbest.pdf', 
    useDingbats = F, height = 25, width = 25)
plotHeatmap(caglayan_primate_subclass_best_hits, cex = .5)
dev.off()



start = Sys.time()
caglayan_primate_subclass_aurocs = MetaNeighborUS(var_genes = global_hvgs, 
                              dat = merged_caglayan_primate, 
                              study_id = merged_caglayan_primate$study_id,
                              cell_type = merged_caglayan_primate$primate_subclass_primate_cluster,
                              fast_version = T)
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_primate_subclass_aurocs.pdf', 
    useDingbats = F, height = 25, width = 25)
plotHeatmap(caglayan_primate_subclass_aurocs, cex = .2)
dev.off()
...
saveRDS(caglayan_primate_subclass_best_hits, 
        file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_primate_subclass_1vsbestMN.rds')

saveRDS(caglayan_primate_subclass_aurocs, 
        file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_primate_subclass_full_aurocs.rds')

```

```{r}

caglayan_primate_subclass_best_hits = readRDS(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_primate_subclass_1vsbestMN.rds')

caglayan_primate_subclass_aurocs = readRDS(file =
                                             '/home/werner/projects/mouse_brain_consensus_taxonomy/data/metaneighbor/non_allen_data/caglayan_vs_primate_subclass_full_aurocs.rds')
...
cluster_graph = makeClusterGraph(caglayan_primate_subclass_best_hits , low_threshold = .6)

plotClusterGraph(cluster_graph, merged_caglayan_primate$study_id, 
                 merged_caglayan_primate$primate_subclass_primate_cluster, size_factor = 3)
...
primate_subclass_mappings_df = as.data.frame(colData(merged_caglayan_primate)) %>% filter(!grepl('caglayan', study_id)) %>%
  mutate(study_clust_label = paste(study_id, primate_subclass_primate_cluster, sep = '|')) %>%
  filter(!duplicated(study_clust_label))
...
pdf(file = '/home/werner/projects/mouse_brain_consensus_taxonomy/graphs/non_allen_data/caglayan_vs_primate_subclass_1vsbest_cluster_graph.pdf', 
    useDingbats = F, height = 10, width = 10)
plot(temp_cluster_graph,
     vertex.size = 8,
     vertex.label.cex=1.5,
...
And what about just highlighting the wackyness of cluster 20?


```{r}

test_refs = c('human_10x|Vip','human_ss|Vip','chimp_10x|Vip','chimp_ss|Vip',
              'gorilla_10x|Vip','gorilla_ss|Vip', 'macaque_10x|Vip','marmoset_10x|Vip')

r_index = rownames(caglayan_primate_subclass_aurocs) %in% test_refs
c_index = grepl('caglayan', colnames(caglayan_primate_subclass_aurocs))

cag_clust_vip_aurocs = colMeans(caglayan_primate_subclass_aurocs[r_index, c_index])


test_refs = c('human_10x|Sncg','human_ss|Sncg','chimp_10x|Sncg','chimp_ss|Sncg',
              'gorilla_10x|Sncg','gorilla_ss|Sncg', 'macaque_10x|Sncg','marmoset_10x|Sncg')

r_index = rownames(caglayan_primate_subclass_aurocs) %in% test_refs
c_index = grepl('caglayan', colnames(caglayan_primate_subclass_aurocs))

cag_clust_sncg_aurocs = colMeans(caglayan_primate_subclass_aurocs[r_index, c_index])


temp_df = data.frame(cag_clust_vip_aurocs = cag_clust_vip_aurocs, cag_clust_sncg_aurocs = cag_clust_sncg_aurocs )


temp_df$cluster = sapply(strsplit(rownames(temp_df), split = "|", fixed = T), '[[', 2)

temp_df = temp_df %>% group_by(cluster) %>% summarise(mean_vip_auroc = mean(cag_clust_vip_aurocs), 
                                            mean_sncg_auroc = mean(cag_clust_sncg_aurocs))


ggplot(temp_df, aes(x = mean_vip_auroc, y = mean_sncg_auroc, label = cluster)) + geom_point(size = 3) +
  geom_label_repel(min.segment.length = 0, force = .5, nudge_x = .025) +
  geom_abline(slope = 1, intercept = 0, color = 'red', linetype = 'dashed') +
  ylim(.7,1) + xlim(.7, 1) + theme_bw() + xlab('Average Primate Vip AUROC') + ylab('Average Primate Sncg AUROC') +
  ggtitle('Caglayan vs Primate MetaNeighbor')
...
test_refs = c('tasic18|Vip','zeng_10x_cells|Vip','zeng_10x_nuclei|Vip','zeng_smart_cells|Vip',
              'zeng_smart_nuclei|Vip','zeng_10xv2_wba|Vip', 'zeng_10xv3_wba|Vip','macosko_wba|Vip')

r_index = rownames(caglayan_mouse_subclass_aurocs) %in% test_refs
c_index = grepl('caglayan', colnames(caglayan_mouse_subclass_aurocs))

cag_clust_vip_aurocs = colMeans(caglayan_mouse_subclass_aurocs[r_index, c_index])


test_refs = c('tasic18|Sncg','zeng_10x_cells|Sncg','zeng_10x_nuclei|Sncg','zeng_smart_cells|Sncg',
              'zeng_smart_nuclei|Sncg','zeng_10xv2_wba|Sncg', 'zeng_10xv3_wba|Sncg','macosko_wba|Sncg')

r_index = rownames(caglayan_mouse_subclass_aurocs) %in% test_refs
c_index = grepl('caglayan', colnames(caglayan_mouse_subclass_aurocs))

cag_clust_sncg_aurocs = colMeans(caglayan_mouse_subclass_aurocs[r_index, c_index])
...
ggplot(temp_df, aes(x = mean_vip_auroc, y = mean_sncg_auroc, label = cluster)) + geom_point(size = 3) +
  geom_label_repel(min.segment.length = 0, force = .5, nudge_x = .025) +
  geom_abline(slope = 1, intercept = 0, color = 'red', linetype = 'dashed') +
  theme_bw() + xlab('Average Mouse Vip AUROC') + ylab('Average Mouse Sncg AUROC') +
  ggtitle('Caglayan vs Mouse MetaNeighbor')
...
test_mat = matrix(c(1:10), nrow = 2, dimnames = list(c('A','B'),c('C','D', 'E','F','G')))

test_mat

reshape2::melt(test_mat)
