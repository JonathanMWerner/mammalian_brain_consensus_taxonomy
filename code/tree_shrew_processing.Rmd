...
load up GTF file and get gene annotations

```{r}

gtf_file = '/inkwell03/werner/sc_data/tree_shrew_xiong_2025/TS_3.0.genomeannotation.gtf.gz'


tree_shrew_gtf =rtracklayer::import(gtf_file)
tree_shrew_gtf = as.data.frame(tree_shrew_gtf)

table(tree_shrew_gtf$type)

#Data gene names seem to match with the gene_id
tree_shrew_gtf %>% filter(type == 'transcript')


index = match(rownames(tree_shrew_sce), tree_shrew_gtf$gene_id)
tree_shrew_gtf[index, ]


rownames(tree_shrew_sce) = rowData(tree_shrew_sce)$Symbol

table(tree_shrew_gtf$seqnames)
...
tree_shrew_sce$glutamatergic_enrichment = ct_enrichment['all|ExN', ]
tree_shrew_sce$GABAergic_enrichment = ct_enrichment['all|InN', ]
tree_shrew_sce$non_neuronal_enrichment = ct_enrichment['all|NonN', ]
...
table(interneuron.combined$Age, interneuron.combined$seurat_clusters )
...
top_markers = mouse_class_meta_markers %>% filter(rank <= 50)
...
top_markers = primate_class_meta_markers %>% filter(rank <= 50)
ct_scores = score_cells(log1p(interneuron.combined@assays$originalexp@data), top_markers)
ct_enrichment = compute_marker_enrichment(ct_scores)
ct_pred = assign_cells(ct_scores)
...
DimPlot(interneuron.combined.filt, reduction = "umap", group.by = 'Age')
DimPlot(interneuron.combined.filt, reduction = "umap", group.by = 'seurat_clusters')
DimPlot(interneuron.combined.filt, reduction = "umap", group.by = 'seurat_clusters', label = T)
DimPlot(interneuron.combined.filt, reduction = "umap", group.by = 'mouse_cge_mge', label = T)
DimPlot(interneuron.combined.filt, reduction = "umap", group.by = 'primate_cge_mge', label = T)
...
top_markers = all_meta_markers %>% filter(rank <= 100)
...
#top_mouse_sncg = mouse_subclass_meta_markers %>% filter(cell_type == 'Sncg' & rank <= 50) %>% pull(gene)
...
#tree_shrew_inhibitory_seurat$agg_mouse_sncg_markers = scale(rowSums(FetchData(tree_shrew_inhibitory_seurat, vars = top_mouse_sncg, layer = 'data')))
#tree_shrew_inhibitory_seurat$agg_primate_sncg_markers = scale(rowSums(FetchData(tree_shrew_inhibitory_seurat, vars = top_primate_sncg, layer = 'data')))
#tree_shrew_inhibitory_seurat$agg_primate_vip_markers = scale(rowSums(FetchData(tree_shrew_inhibitory_seurat, vars = top_primate_vip, layer = 'data')))
#tree_shrew_inhibitory_seurat$agg_primate_ancestral_sncg_markers = scale(rowSums(FetchData(tree_shrew_inhibitory_seurat, 
...
DimPlot(tree_shrew_inhibitory_seurat, reduction = "umap", label = T)
DimPlot(tree_shrew_inhibitory_seurat, reduction = "umap", group.by = 'primate_cge_mge')
...
mouse_markers = top_markers %>% filter(cell_type =='mouse_Vip') %>% pull(gene)
primate_markers = top_markers %>% filter(cell_type =='Vip') %>% pull(gene)


table(mouse_markers %in% primate_markers)
...
cluster_filt = tree_shrew_inhibitory_seurat$seurat_clusters %in% c(1, 11)

column_color = tree_shrew_inhibitory_seurat$seurat_clusters[cluster_filt]

cge_enrichs = tree_shrew_inhibitory_seurat[[]][cluster_filt , c('enrich_mouse_sncg','enrich_mouse_vip',
                                                                'enrich_primate_sncg','enrich_primate_vip', 'enrich_primate_ancestral_sncg') ]

cge_enrichs$cluster = column_color

mean_enrichs = cge_enrichs %>% group_by(cluster) %>% summarise(mean_mouse_sncg = mean(enrich_mouse_sncg), mean_mouse_vip = mean(enrich_mouse_vip),
                                                mean_primate_sncg = mean(enrich_primate_sncg), mean_primate_vip = mean(enrich_primate_vip),
                                                mean_primate_ancestral_sncg = mean(enrich_primate_ancestral_sncg)) %>% as.data.frame()

rownames(mean_enrichs) = mean_enrichs$cluster
mean_enrichs = mean_enrichs %>% select(-c(cluster))
mean_enrichs = as.matrix(mean_enrichs)

max_norm_enrichs = cge_enrichs / rowMaxs(cge_enrichs)

col_annot = ComplexHeatmap::HeatmapAnnotation(tree_shrew_cluster = column_color)

col_fun = circlize::colorRamp2(breaks = c(0,1, 2), colors = c('white','orange','red'))
ComplexHeatmap::Heatmap(mean_enrichs, name = 'marker_enrichment',
                        col = col_fun)
